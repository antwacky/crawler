...
if __name__ == "__main__":
    ...
    parser.add_argument('--pools', '-p', type=int, dest='pools', help='pools to use', required=False)
    parser.add_argument('--quiet', '-q', action='store_true', dest='quiet', help='hide progress', required=False)

    args = parser.parse_args()

    spider = Spider(domain=args.domain, start_url=args.starturl, exclusive=args.exclusive, pools=args.pools, quiet=args.quiet)
    result = spider.crawl()
